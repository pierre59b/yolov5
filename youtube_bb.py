...
d_sets = [
          'youtube_boundingboxes_detection_validation'
...
def dl_and_cut(annot,vid):
  ...
  check_call(['if', 'not', 'exist', frame_dir, 'mkdir', frame_dir], shell=True)
...
def sched_downloads(annot, d_set,dl_dir,num_threads,vids):
  d_set_dir = dl_dir+'\\'+d_set+'\\'
  ...
  with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
    fs = [executor.submit(dl_and_cut,[an for an in annot if an[0]==vid.yt_id],vid) for vid in vids]
